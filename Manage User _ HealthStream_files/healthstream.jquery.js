...
(function ($) {
    ...
    $.fn.center = function (target) {
        ...
        if (targetObj.exists()) {
            ...
            if (targetObj.offset()) {
                targetOffsetTop = targetObj.offset().top;
                targetOffsetLeft = targetObj.offset().left;
            }
            this.css("top", Math.max(0, ((targetObj.height() - self.outerHeight()) / 2) + targetOffsetTop) + "px");
            this.css("left", Math.max(0, ((targetObj.width() - self.outerWidth()) / 2) + targetOffsetLeft) + "px");
        } else {
    ...
    $.fn.reBind = function (types, selector, fn) {
        return this.off(types, selector, fn).on(types, selector, fn);
    };
