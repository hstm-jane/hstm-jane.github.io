    ...
    $.fn.stiffChart = function( options ) {
      ...
      if ( settings.childCounter ) {
        $('[data-parent]').each(function(){
          ...
          if (count !== 0) {
            if (count === 1) {
                $(this).children('.the-chart').append('<div class="total-child">' + count + ' direct report</div>');
            }
            else {
                $(this).children('.the-chart').append('<div class="total-child">' + count + ' direct reports</div>');
            }
          }
        });
      }
