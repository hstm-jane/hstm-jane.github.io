        ...
        if ( !empty($li->type) ) {
            switch(strtolower($li->type)){
                ...
                case 'learning event':
                    ...
                    break;
                case 'student search':
                    $icon = '<i class="fa fa-search fa-lg"></i> ';
                    break;
                case 'assessment':
...
function get_action ( $data ) {
    ...
    switch ( $data->type ) {
        case "link":
            ...
            break;
        case "link-danger":
            $action .= '<a class="btn btn-sm btn-danger"' . get_attributes( $data->attributes ) . '>' . $data->text . ' </a>' . "\n";
            break;
        case "link-success":
            $action .= '<a class="btn btn-sm btn-success"' . get_attributes( $data->attributes ) . '>' . $data->text . ' <i class="fa fa-plus fa-lg visible-xs"></i></a>' . "\n";
            break;
        case "link-add":
            $action .= '<a style="max-width: 78px" class="btn btn-sm btn-default pull-right btn-add-all"' . get_attributes( $data->attributes ) . '>' . $data->text . $mobile_icon .'</a>' . "\n";
            break;
        case "link-locked":
