...
var defaults = {
	...
	// events
	 ...
	 // events: [
  //               { start: moment().subtract({'d':4}), end: moment().subtract({'d':23,'h':20}), title: 'This is an event', color: '1', id: 'event-1' },
  //               { start: moment(), end: moment().add({'h':1}), title: 'This is an event', color: '1', id: 'event-2' },
  //               { start: moment().add({'h':3}), end: moment().add({'h':3}), title: 'This is an event', color: '1', id: 'event-3' },
  //               { start: moment().add({'h':3}), end: moment().add({'h':2}), title: 'This is an event', color: '1', id: 'event-4' },
  //               { start: moment().add({'d':1}), end: moment().add({'d':1,'h':1}), title: 'This is another event', color: '2', id: 'event-5' }
  //           ],
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2 class="clndr-date" style="cursor: pointer;">&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
...
function EventManager(options) { // assumed to be a calendar
	...
	function mutateEvent(event, props) {
		...
		if (!clearEnd && props.end) {
            ...
            // #edit 3-18-2015 jgreer
			var durationMilliseconds = durationDelta.asMilliseconds();
		    durationDelta = moment.duration(durationMilliseconds);
		}
				...
				'<tbody>' +
					'<tr>' +
						'<td class="' + this.widgetContentClass + '">' +
							'<div class="fc-day-grid-container" style="min-height:336px !important">' +
