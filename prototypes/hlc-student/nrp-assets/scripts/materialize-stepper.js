...
if (validation) {
   $.validator.setDefaults({
      ...
      errorPlacement: function (error, element) {
         ...
         else if($(element).attr('id') == "GoalStartDate" || $(element).attr('id') == "GoalEndDate") {
            var parentElem = $(element).parents('.date');
             error.insertAfter(parentElem);
          }
          else if($(element).attr('id') == "resource-link-url") {
...
$.fn.activateFeedback  = function() {
   ...
   active.eq(0).prepend('<div class="wait-feedback"> <i class="fa fa-spinner fa-spin fa-4x"></i></div>');
   active.eq(1).prepend('<div class="wait-feedback"> </div>');
};
...
$.fn.resetStepper  = function(step) {
   ...
   return $(this).openStep(step, undefined, activefb);
};
...
$.fn.nextStep = function(callback, activefb, e) {
   ...
   if(form.isValid()) {
      ...
      //    return window[feedback](param);
      // }
      active.removeClass('wrong').addClass('done');
      this.openStep(next, callback, activefb, next);
...
$.fn.prevStep = function(callback, activefb, e) {
  stepper = this;
  settings = $(stepper).data('settings');
  form = this.closest('form');
   active = this.find('.step.active');
   ...
   feedback = active.find('.previous-step').length > 1 ? (e ? $(e.target).data("feedback") : undefined) : active.find('.previous-step').data("feedback");
   if(form.isValid()) {
      // if(feedback && activefb) {
      //   var param = 'previous';
      //    if(settings.showFeedbackLoader) stepper.activateFeedback();
      ...
      // }
      active.removeClass('wrong').addClass('done');
      this.openStep(prev, callback, activefb, prev);
...
$.fn.openStep = function(step, callback, activefb, nextStep) {
  $stepper = $('.stepper');
  ...
  feedback = active.find('.previous-step').length > 1 ? (e ? $(e.target).data("feedback") : undefined) : active.find('.previous-step').data("feedback");
    ...
    if(form.isValid()) {
     active.removeClass('wrong').addClass('done');
      if(feedback && activefb) {
        if (nextStep > prev_active) {
         var param = 'next';
        }
        else if (nextStep > prev_active) {
          var param = 'previous';
        }
        else {
            var param = nextStep;
...
$.fn.closeAction = function(order, callback) {
   ...
   if(window.innerWidth < 993 || !this.closest('ul').hasClass('horizontal')) {
    if(closable.length > 0) {
      closable.eq(0).stop().slideUp(300,"easeOutQuad", callback);
    }
   } else {
    if(closable.length > 0) {
      if(order==1) {
         closable.eq(0).animate({left: '-100%'},function(){closable.css({display: 'none', left: '0%'}, callback);});
      } else {
         closable.eq(0).animate({left: '100%'},function(){closable.css({display: 'none', left: '0%'}, callback);});
      }
    }
...
$.fn.openAction = function(order, callback) {
   ...
   openable = this.addClass('active').addClass('canclick').find('.step-content');
   this.next('.step').addClass('canclick').find('.step-title').addClass('waves-effect');
   if(window.innerWidth < 993 || !this.closest('ul').hasClass('horizontal')) {
...
$.fn.activateStepper = function(options, window) {
   var settings = $.extend({
   ...
   $(this).each(function(){
      ...
      $stepper.on("click", '.step:not(.active)', function () {
         ...
         prev_active = $($stepper.children('.step:visible')).index(active);
         if(!$stepper.hasClass('linear')) {
            $stepper.openStep(object+1, undefined, false);
         } else if(settings.linearStepsNavigation) {
            active = $stepper.find('.step.active');
            if($('.stepper').children('.step:visible').eq(object).hasClass('done') || $('.stepper').children('.step:visible').eq(object).prev('.step').hasClass('done')) {
                $stepper.openStep(object+1, undefined, true, (object + 1));
            } else if($($stepper.children('.step:visible')).index($(active))+1 == object) {
               $stepper.nextStep(undefined, true, undefined);
            } else if ($($stepper.children('.step:visible')).index($(active))-1 == object) {
               $stepper.prevStep(undefined, true, undefined);
            }
         }
         ...
         e.preventDefault();
         $stepper.prevStep(undefined, true, e);
